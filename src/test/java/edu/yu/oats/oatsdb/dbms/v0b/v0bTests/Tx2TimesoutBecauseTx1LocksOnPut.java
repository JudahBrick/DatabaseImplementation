 ...
 * Tx2 blocks when trying to do a "put()" on the same MapEntry.  This is
 * manifested by Tx2 timing out if Tx1 takes too long to commit.
 *
 * Note: The DBMS currently allows clients to sleep once, attempt to acquire
 * the lock & fail, and only then rollback the tx.  So, Tx2 will be rolled back
 * only after TWO timeout periods have passed.
 *
 * @author Avraham Leff
 */
...
public class Tx2TimesoutBecauseTx1LocksOnPut {

    private Tx2TimesoutBecauseTx1LocksOnPut() throws Exception {
    ...
    private static class Tx1 implements Runnable {
        Tx1(final Tx2TimesoutBecauseTx1LocksOnPut outer) {
        ...
        public void run() {
            try {
                ...
                logger.debug("Tx {} inserted account {}", _id, account);
                ...
                logger.debug("Tx {}: now sleeping for 'more than' tx timeout "+
                        "duration {}", _id, moreThanTimeout);
        ...
        private final Tx2TimesoutBecauseTx1LocksOnPut _outer;
    }

    private static class Tx2 implements Runnable {
        Tx2(final Tx2TimesoutBecauseTx1LocksOnPut outer) {
            Objects.requireNonNull(outer, "outer");
            _outer = outer;
            _id = "Tx2";
        }
        ...
        public void run() {
            try {
                ...
                _outer._savings.put
                        (accountId,
                                new Account(accountId, randomString.next(), 140, SAVINGS));
    ...
    public static void main(final String[] args) throws Exception {
        ...
        final Tx2TimesoutBecauseTx1LocksOnPut test =
                new Tx2TimesoutBecauseTx1LocksOnPut();
    ...
    private static int moreThanTimeout() {
        return dbms.getTxTimeoutInMillis() + 50;
    }
    ...
    private final static Logger logger =
            LogManager.getLogger(Tx2TimesoutBecauseTx1LocksOnPut.class);
    ...
    final static int timeout = 1000; // 1 second in ms
    ...
    private Account _account;     // set by Tx1
}
