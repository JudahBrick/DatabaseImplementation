 ...
 * currently locked by Tx1.  Then, Tx1 initiates a rollback, relinquishes the
 * lock in time to prevent Tx2 from rolling back.
 *
 * @author Avraham Leff
 */
...
public class Tx1RollsbackTx2Commits {

    private Tx1RollsbackTx2Commits() throws Exception {
    ...
    private static class Tx1 implements Runnable {
        Tx1(final Tx1RollsbackTx2Commits outer) {
        ...
        public void run() {
            try {
                ...
                logger.debug("Tx {}: waking up, and rolling back tx", _id);
                txMgr.rollback();
            }
            catch(Exception e) {
        ...
        private final Tx1RollsbackTx2Commits _outer;
    }

    private static class Tx2 implements Runnable {
        Tx2(final Tx1RollsbackTx2Commits outer) {
            Objects.requireNonNull(outer, "outer");
            _outer = outer;
            _id = "Tx2";
        }
    ...
    public static void main(final String[] args) throws Exception {
        ...
        final Tx1RollsbackTx2Commits test = new Tx1RollsbackTx2Commits();
    ...
    private final static Logger logger =
            LogManager.getLogger(Tx1RollsbackTx2Commits.class);
